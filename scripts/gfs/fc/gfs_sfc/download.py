...
def download_gfs(yaml_file):
    ...
    for date in dates:
        ...
        for gfs_file_type in gfs_file_types:
            file_name = f'{day_start}.{window_start}.{gfs_file_type}'
            source = f's3://noaa-gfs-warmstart-pds/gfs.{day}/{hour}/RESTART/{file_name}'
            target = f'file://{download_dir}/gfs.{day}/{hour}/RESTART/{file_name}'
