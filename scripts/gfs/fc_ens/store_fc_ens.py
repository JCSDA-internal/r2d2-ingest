    ...
    for date in dates:
        ...
        for member in range(first_member, last_member+1):
            for file_type in file_types:
                filename = f'file://{source_dir}/enkfgdas.{window_begin.year()}{window_begin.month()}{window_begin.day()}/{window_begin.hour()}/atmos/mem{str(member).zfill(3)}/RESTART/{window_mid.year()}{window_mid.month()}{window_mid.day()}.{window_mid.hour()}0000.$(file_type).tile$(tile).nc'
                store(
                    model=model,
                    type=type,
